...
sys.path.insert(0, r"D:\SmallCap")  # Add SmallCap repo path
...
# Import SmallCap model (requires cloning the SmallCap repository)
try:
    from smallcap import SmallCapModel, SmallCapProcessor
    SMALLCAP_AVAILABLE = True
    print("✓ SmallCap model imported successfully")
except ImportError as e:
    SMALLCAP_AVAILABLE = False
    print(f"✗ SmallCap import failed: {e}")
    print("Please clone the SmallCap repository from: https://github.com/RitaRamo/smallcap")
...
class TripletShift:
    """
    TripletShift (TS) Attack for SmallCap
    """
    ...
    def __init__(self,
                 ...
                 lambda_p=0.1,
                 mu=0.001,
                 smallcap_checkpoint_path=None,  # Path to SmallCap checkpoint
                 device="cuda"):
        ...
        # Initialize victim SmallCap model
        self._load_victim_model(smallcap_checkpoint_path)

        # Image transforms (CLIP normalization for the surrogate encoder)
        ...
        # SmallCap may use different transforms - check the repo for exact requirements
        # Typically: resize to 224 or 384, normalize with CLIP stats
        self.smallcap_transform = transforms.Compose([
        ...
        print(f"\n{'='*60}")
        print(f"TripletShift Attack with SmallCap Victim Model")
        print(f"{'='*60}")
        print(f"SmallCap Checkpoint: {smallcap_checkpoint_path}")
    ...
    def _load_victim_model(self, checkpoint_path):
        """Load SmallCap victim model"""
        if not SMALLCAP_AVAILABLE:
            raise RuntimeError(
                "SmallCap is not available. Please clone the repository:\n"
                "git clone https://github.com/RitaRamo/smallcap.git\n"
                "Then add it to your Python path."
            )

        print(f"Loading SmallCap victim model from {checkpoint_path}...")

        try:
            # Load SmallCap model
            # The exact loading method depends on how SmallCap is structured
            # Based on the paper: CLIP encoder + GPT-2 decoder with cross-attention

            self.victim_model = SmallCapModel.from_pretrained(checkpoint_path)
            self.victim_processor = SmallCapProcessor.from_pretrained(checkpoint_path)
            ...
            print(f"✓ SmallCap model loaded successfully! ({num_params:.1f}M parameters)\n")

        except Exception as e:
            print(f"Error loading SmallCap model: {e}")
            print("\nTroubleshooting:")
            print("1. Make sure you have downloaded the SmallCap checkpoint from the official repository")
            print("2. The expected checkpoint path should contain model weights")
            print("3. You may need to use a specific loading method from the SmallCap codebase")
            raise
    ...
    def generate_caption(self, image_tensor):
        """
        Generate caption using SmallCap victim model.

        SmallCap uses retrieval augmentation - it retrieves similar captions
        from a datastore to condition the generation.
        """
        # Denormalize image
        ...
        # Apply SmallCap-specific transforms
        image_input = self.smallcap_transform(image_pil).unsqueeze(0).to(self.device)

        with torch.no_grad():
            # SmallCap generation with retrieval
            # Note: You may need to provide a datastore for retrieval
            # For zero-shot/generalization, SmallCap can work without retrieval
...
def main():
    ...
    output_dir = r"D:\ts_output_smallcap"

    # IMPORTANT: Update this path to your SmallCap checkpoint
    smallcap_checkpoint_path = r"D:\SmallCap\checkpoints\smallcap_coco.pt"

    os.makedirs(output_dir, exist_ok=True)

    if not SMALLCAP_AVAILABLE:
        print("\n" + "="*60)
        print("ERROR: SmallCap model not available")
        print("="*60)
        print("\nTo use SmallCap, you need to:")
        print("1. Clone the repository: git clone https://github.com/RitaRamo/smallcap.git")
        print("2. Download the pre-trained checkpoint from the official repository")
        print("3. Update the 'smallcap_checkpoint_path' variable with the correct path")
        print("4. Add the SmallCap directory to your Python path")
        print("\nThe SmallCap paper: https://arxiv.org/abs/2209.15323")
        print("Official code: https://github.com/RitaRamo/smallcap")
        return

    # Initialize TripletShift attack with SmallCap
    attack = TripletShift(
        epsilon=8/255,
        gamma_step=1/255,
        K=100,
        ...
        lambda_p=0.1,
        mu=0.001,
        smallcap_checkpoint_path=smallcap_checkpoint_path,
    ...
    for idx, (img_file, clean_cap, target_cap) in enumerate(
        tqdm(zip(image_files, clean_captions, target_captions), total=num_samples, desc="TripletShift Attack with SmallCap")):
        ...
        try:
            ...
            # Generate caption using SmallCap
            ...
            print(f"GENERATED (SmallCap): {generated_caption[:80]}...")
            print(f"{'='*50}")
    ...
    if results['bleu1']:
        print("\n" + "="*70)
        print("TripletShift ATTACK - FINAL RESULTS (SmallCap Victim)")
        print("="*70)
        ...
        with open(os.path.join(output_dir, "ts_results_smallcap.json"), 'w') as f:
