...
from transformers import (
    LlavaProcessor,           # LLaVA processor
    LlavaForConditionalGeneration  # LLaVA model
)
...
class TripletShift:
    """
    TripletShift (TS) Attack with LLaVA (7B) Victim Model
    ...
    def __init__(self,
                 ...
                 llava_model_name="llava-hf/llava-1.5-7b-hf",  # LLaVA 7B model
                 device="cuda"):
        """
        Initialize TripletShift attack with LLaVA victim model.
        ...
        Args:
            ...
            llava_model_name: LLaVA model variant to use
        ...
        print(f"\n{'='*60}")
        print(f"TripletShift Attack with LLaVA (7B) Victim Model")
        print(f"{'='*60}")
        print(f"LLaVA Model: {llava_model_name}")
    ...
    def _load_victim_model(self):
        """Load LLaVA victim model from HuggingFace"""
        print(f"Loading LLaVA (7B) victim model: {self.llava_model_name}...")

        # LLaVA model variants:
        # - "llava-hf/llava-1.5-7b-hf" (LLaVA 1.5 7B)
        # - "llava-hf/llava-1.5-13b-hf" (LLaVA 1.5 13B)
        # - "llava-hf/llava-interleave-qwen-0.5b-hf" (smaller variant)

        try:
            # Load LLaVA processor and model
            self.victim_processor = LlavaProcessor.from_pretrained(self.llava_model_name)
            self.victim_model = LlavaForConditionalGeneration.from_pretrained(
            ...
            print(f"Error loading {self.llava_model_name}: {e}")
            print("Trying alternative LLaVA model...")
        ...
        # Get model size
        num_params = sum(p.numel() for p in self.victim_model.parameters()) / 1e9
        print(f"✓ LLaVA model loaded successfully! ({num_params:.1f}B parameters)\n")
    ...
    def generate_caption(self, image_tensor):
        """
        Generate caption using LLaVA victim model.
        LLaVA requires a prompt for image captioning.
        """
        # Denormalize image
        ...
        # LLaVA requires a prompt for generation
        # Standard prompt for image captioning
        ...
        full_response = self.victim_processor.decode(outputs[0], skip_special_tokens=True)
...
def main():
    ...
    output_dir = r"D:\ts_output_llava"  # Changed output directory

    os.makedirs(output_dir, exist_ok=True)

    # LLaVA model variants:
    # - "llava-hf/llava-1.5-7b-hf" (7B model, recommended)
    # - "llava-hf/llava-1.5-13b-hf" (13B model, requires more memory)
    # - "llava-hf/llava-interleave-qwen-0.5b-hf" (smaller variant for testing)

    # Initialize TripletShift attack with LLaVA
    attack = TripletShift(
        ...
        llava_model_name="llava-hf/llava-1.5-7b-hf",  # LLaVA 7B
    ...
    for idx, (img_file, clean_cap, target_cap) in enumerate(
        tqdm(zip(image_files, clean_captions, target_captions), total=num_samples, desc="TripletShift Attack with LLaVA")):
        ...
        try:
            ...
            print(f"GENERATED (LLaVA): {generated_caption[:80]}...")
            print(f"{'='*50}")
    ...
    if results['bleu1']:
        print("\n" + "="*70)
        print("TripletShift ATTACK - FINAL RESULTS (LLaVA 7B Victim)")
        print("="*70)
        ...
        final_results = {
            'model': 'LLaVA-7B',
            'llava_model_variant': attack.llava_model_name,
            'metrics': {
        ...
        with open(os.path.join(output_dir, "ts_results_llava.json"), 'w') as f:
