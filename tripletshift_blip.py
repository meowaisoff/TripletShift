...
class TripletShift:
    ...
    def __init__(self,
        ...
        print(f"\n{'='*60}")
        print(f"TripletShift Attack Initialized")
        print(f"{'='*60}")
    ...
    def _load_victim_model(self):
        """Load BLIP victim model from HuggingFace"""
        print("Loading BLIP victim model...")
        try:
            # Try to load from cache first
            cache_dir = os.path.expanduser("~/.cache/huggingface/hub/models--Salesforce--blip-image-captioning-base")
            self.victim_processor = BlipProcessor.from_pretrained(
                "Salesforce/blip-image-captioning-base",
                cache_dir=cache_dir
            )
            self.victim_model = BlipForConditionalGeneration.from_pretrained(
                "Salesforce/blip-image-captioning-base",
                cache_dir=cache_dir
            )
        except:
    ...
    def generate_caption(self, image_tensor):
        """Generate caption using BLIP victim model"""
        # Denormalize image
...
def main():
    ...
    for idx, (img_file, clean_cap, target_cap) in enumerate(
        tqdm(zip(image_files, clean_captions, target_captions), total=num_samples, desc="TripletShift Attack")):
    ...
    if results['bleu1']:
        print("\n" + "="*70)
        print("TripletShift ATTACK - FINAL RESULTS (BLIP Victim)")
        print("="*70)
        ...
        with open(os.path.join(output_dir, "ts_results_blip.json"), 'w') as f:
